...
class LightHamHead(nn.Module):
    def __init__(self, ham_norm_cfg, dropout_ratio=0., ham_channels=512, factor=2, ham_kwargs=dict()):
        ...
        self.conv1 = nn.Conv2d(hidden_size, 2, 1)

    def forward(self, x_c4, x_c3, x_c2, x_c1, l):
        # fuse Y4, Y3, Y2
        x4 = resize(x_c4, size=x_c1.shape[2:], mode='bilinear', align_corners=self.align_corners)
        x3 = resize(x_c3, size=x_c1.shape[2:], mode='bilinear', align_corners=self.align_corners)
        x2 = resize(x_c2, size=x_c1.shape[2:], mode='bilinear', align_corners=self.align_corners)
        
        x = torch.cat([x4, x3, x2, x_c1], dim=1)
        x = self.squeeze(x)
